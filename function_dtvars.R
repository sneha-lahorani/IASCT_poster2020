...
one <- {{dsetin}} %>%
    mutate(.len_ = str_length({{datetmchar}}),
...
two <- one %>% filter(.token_ == "FDT") %>%
  mutate(dtm = strptime(chardate1, format="%Y-%m-%d %H:%M"),
         dt = as.Date(chardate2,"%Y-%m-%d"),
         tm = times(chartime))

three <- one %>% filter(.token_ == "FD") %>%
  mutate(dt = as.Date(chardate2,"%Y-%m-%d"),
         dtm = as.POSIXct(NA))

four <- one %>% filter(.token_ == "PD") %>%
  mutate(dt = as.Date(NA),
         dtm = as.POSIXct(NA))

final <- bind_rows(two, three, four)
if (any(is.na(final))) {warning("Input datetime character variable contains partial datetime values.")}
...
birthdates <- data.frame(BIRTHDTC = c("2018-12-31", "1997-07-15T07:55", "2007-09-19T00:02","1995-12-09T13:30","2020-11", "1986"),
